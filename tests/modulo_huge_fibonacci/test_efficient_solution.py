...
from solutions.modulo_huge_fibonacci.efficient_solution import _get_pisano_period
from tests.modulo_huge_fibonacci.stress_tests_params import StressTestsParams
...
def test_efficient_solution_simple_cases(mod: int):
    assert 0 % mod == modulo_huge_fibonacci.efficient_solution((0, mod))
    assert 1 % mod == modulo_huge_fibonacci.efficient_solution((1, mod))
    assert 1 % mod == modulo_huge_fibonacci.efficient_solution((2, mod))
    assert 2 % mod == modulo_huge_fibonacci.efficient_solution((3, mod))
    assert 3 % mod == modulo_huge_fibonacci.efficient_solution((4, mod))
    assert 5 % mod == modulo_huge_fibonacci.efficient_solution((5, mod))
    assert 8 % mod == modulo_huge_fibonacci.efficient_solution((6, mod))
    assert 13 % mod == modulo_huge_fibonacci.efficient_solution((7, mod))
...
names = ["modulo_huge_fibonacci", "stress_tests"]
...
def sample_input_fibonacci(params: StressTestsParams) -> List[Tuple[int, int]]:
    ...
    min_modulus = params.min_modulo
    max_modulus = params.max_modulo
...
def test_stress_tests_efficient_solution_small_stress(input_fibonacci):
    brute_force = modulo_huge_fibonacci.brute_force_solution(input_fibonacci)
    efficient = modulo_huge_fibonacci.efficient_solution(input_fibonacci)
...
def test_performance(input_fibonacci) -> None:
    config_names = ["modulo_huge_fibonacci", "performance_tests"]
    ...
    brute_force_time = performance_measures.measure_performance(
        modulo_huge_fibonacci.brute_force_solution,
        input_fibonacci,
    )
    efficient_time = performance_measures.measure_performance(
        modulo_huge_fibonacci.efficient_solution,
        input_fibonacci,
    )
